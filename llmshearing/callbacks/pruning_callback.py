...
class PruningCallback(Callback):
    ...
    def plug_in_pruned_steps(self, state: State, logger: Logger):
        ...
        if getattr(state.model.model, "l0_module", None) is not None:
            ...
            state.batch["pruned_steps"] = torch.LongTensor(
                [state.timestamp.batch.value] * len(input_ids)
            ).to(input_ids.device)
    ...
    def after_train_batch(self, state: State, logger: Logger) -> None:
        ...
        if l0_output is not None:
            logger.log_metrics(
                {
                    f"metrics/train/{name}": (
                        val.cpu().item() if torch.is_tensor(val) else val
                    )
                    for (name, val) in l0_output[1].items()
                }
            )
    ...
    def eval_end(self, state: State, logger: Logger) -> None:
        ...
        torch.save(
            zs,
            os.path.join(
                self.save_folder.replace("{run_name}", state.run_name), f"zs_s{step}.pt"
            ),
        )


class CoLACallback(Callback):
