...
def _convert_time(
    time: Union[str, Time[int], Time[float]], state: State, ssr: float = 1.0
) -> Time[int]:
    ...
    if time.unit == TimeUnit.DURATION:
        if state.max_duration.unit == TimeUnit.EPOCH:
            if state.dataloader_len is None:
                raise RuntimeError(
                    "Cannot convert time, as state.dataloader_len is None."
                )
            return Time(
                int(time.value * int(state.dataloader_len) * state.max_duration.value),
                TimeUnit.BATCH,
            )
...
def compile_composer_scheduler(
    schedulers: Sequence[ComposerScheduler], state: State, ssr: float = 1.0
) -> PyTorchScheduler:
    ...
    if len(optimizers) != 1:
        raise NotImplementedError(
            "Providing functional schedulers is unsupported with multiple optimizers."
        )
    optimizer = optimizers[0]
    lambda_scheduler = LambdaLR(
        optimizer,
        [
            lambda epoch, scheduler=scheduler: scheduler(state, ssr)
...
class MultiStepScheduler(ComposerScheduler):
    ...
    def __call__(self, state: State, ssr: float = 1.0):
        milestones = [
            _convert_time(milestone, state, ssr=ssr) for milestone in self.milestones
        ]
...
class CosineAnnealingWarmRestartsScheduler(ComposerScheduler):
    ...
    def __init__(
        self, t_0: Union[str, Time], t_mult: float = 1.0, alpha_f: float = 0.0
    ):
        self.t_0 = t_0
        self.t_mult = t_mult
        self.alpha_f = alpha_f
    ...
    def __call__(self, state: State, ssr: float = 1.0):
        ...
        frac_of_current_interval = (
            (state.timestamp.get(t_0.unit) - current_interval_start)
...
class PolynomialScheduler(ComposerScheduler):
    ...
    def __init__(
        self, power: float, t_max: Union[str, Time] = "1dur", alpha_f: float = 0.0
    ):
        self.t_max = t_max
        self.power = power
        self.alpha_f = alpha_f
...
def _raise_if_warmup_and_max_duration_incompatible(
    t_warmup: Union[str, Time], t_max: Union[str, Time]
):
    ...
    batches_vs_epochs = (
        t_warmup.unit == TimeUnit("ba") and t_max.unit == TimeUnit("ep")
    ) or (t_warmup.unit == TimeUnit("ep") and t_max.unit == TimeUnit("ba"))
...
class MultiStepWithWarmupScheduler(ComposerScheduler):
    ...
    def __init__(
        ...
        self.warmup_scheduler = LinearScheduler(
            alpha_i=0.0, alpha_f=1.0, t_max=t_warmup
        )
    ...
    def __call__(self, state: State, ssr: float = 1.0):
        ...
        _raise_if_warmup_and_max_duration_incompatible(
            self.t_warmup, state.max_duration
        )
...
class ConstantWithWarmupCooldownScheduler(ComposerScheduler):
    r"""Maintains a fixed learning rate, with an initial warmup and a cooldown at end."""

    def __init__(
        self,
        t_warmup: Union[str, Time],
        cooldown_steps: Union[str, Time],
        cooldown_type: str = "sqrt",
        alpha: float = 1.0,
        t_max: Union[str, Time] = "1dur",
        scale_warmup: bool = False,
        **kwargs,
    ...
    def __call__(self, state: State, ssr: float = 1.0) -> float:
        ...
        cooldown_steps = _convert_time(self.cooldown_steps, state)
        t_hold = _convert_time(self.t_max, state) - cooldown_steps

        if state.timestamp < t_hold:
            return self.scheduler(state)

        current_time = state.timestamp.get(t_hold.unit)
        current_factor = 1 - math.sqrt(((current_time - t_hold) / cooldown_steps).value)

        return current_factor


class DelayedConstantWithWarmupCooldownScheduler(ComposerScheduler):
    r"""Maintains a fixed learning rate, with an initial warmup and a cooldown at end."""

    def __init__(
        self,
        t_delay: Union[str, Time],
        t_warmup: Union[str, Time],
        cooldown_steps: Union[str, Time],
        cooldown_type: str = "sqrt",
        alpha: float = 1.0,
    ...
    def __call__(self, state: State, ssr: float = 1.0) -> float:
        ...
        t_delay = _convert_time(self.t_delay, state)
        if state.timestamp < t_delay:
            return 0.0
        ...
        if (current_time - t_delay) < t_warmup:
            return ((current_time - t_delay) / t_warmup).value
        elif current_time < t_hold:
            return 1.0
        else:
            return 1 - math.sqrt(((current_time - t_hold) / cooldown_steps).value)
...
class LinearWithWarmupScheduler(ComposerScheduler):
    ...
    def __call__(self, state: State, ssr: float = 1.0):
        ...
        current_time = state.timestamp.get(t_warmup.unit)
        frac_of_total = (
            ((current_time - t_warmup) / (t_max - t_warmup)).value
            if (t_max > t_warmup)
            else 0.0
        )
