...
from llmshearing.models.l0_module_cola import L0Module
...
class LlamaRMSNorm(nn.Module):
    ...
    def prune_params(self, hidden_z):
        ...
        self.weight = torch.nn.Parameter(
            self.weight.data.mul(hidden_z.squeeze())[remaining_index]
        )
...
class ComposerMosaicCoLA(ComposerModel):
    """Llama model with the Composer model interface."""

    def __init__(self, cfg):
        super().__init__()
        self.model = CoLAModel(cfg)
        self.ref_model = None
        ...
        if self.set_names is not None:
            self.set_name_to_id = {
                set_name: i for i, set_name in enumerate(self.set_names)
            }
            self.set_id_to_name = {
                i: set_name for i, set_name in enumerate(self.set_names)
            }

            for set_name in self.set_names:
                # add train and eval metrics for each set
                self.train_metrics[f"{set_name}_LanguageCrossEntropy"] = (
                    DomainLanguageCrossEntropy(set_name=set_name)
                )
                self.eval_metrics[f"{set_name}_LanguageCrossEntropy"] = (
                    DomainLanguageCrossEntropy(set_name=set_name)
                )
        ...
        self.distill_cola_params_only = getattr(
            cfg.cola_module, "distill_cola_params_only", False
        )
    ...
    def loss(self, outputs, batch):
        ...
        if self.distill_cola_params_only:
            return_loss["total"] = return_loss["distill"]
        else:
...
class CoLAModel(nn.Module):
        ...
        self.transformer.update(
            {
                "blocks": nn.ModuleList(
                    [
                        CoLABlock(cfg, device=cfg.init_device)
    ...
    def prune_params(self, zs=None):
        ...
        if "hidden_z" in zs:
            ...
            self.transformer.output.weight.data = (
                self.transformer.output.weight.data.mul(hidden_z)
            )
...
class CoLABlock(nn.Module):
    def __init__(self, cfg: DictConfig, device: Optional[str] = None):
        ...
        self.ln_1 = layernorm_class(
            cfg.d_model, cfg.get("rms_norm_eps", 1e-6), device=device
        )
        self.attn = CoLAAttention(cfg, device)
        self.ln_2 = layernorm_class(
            cfg.d_model, cfg.get("rms_norm_eps", 1e-6), device=device
        )
    ...
    def forward(
        ...
        mlp_hidden_z: Optional[torch.Tensor] = None,
...
class CoLAAttention(nn.Module):
    ...
    def forward(
        ...
        if self.distillation:
            distill_loss = (
                F.mse_loss(query_s, query_t)
                + F.mse_loss(key_s, key_t)
                + F.mse_loss(value_s, value_t)
            )
        else:
            distill_loss = None
...
class CoLAMLP(nn.Module):
    def __init__(self, cfg: DictConfig, device: Optional[str] = None):
        ...
        if self.calibration:
            self.calib_states = {
                "_".join(ks): None
                for ks in itertools.product(["down_proj", "up_proj"], ["sts", "stu"])
            }
        self.distill_cola_params_only = getattr(
            cola_module_cfg, "distill_cola_params_only", False
        )
...
def normal_attn_fn(query, key, value, attention_mask=None, head_z=None):
...
def flash_attn_fn(
    ...
    if query_padding_mask is None:
        query_padding_mask = torch.ones(
            (batch_size, seqlen), dtype=torch.bool, device=query.device
        )
    if key_padding_mask is None:
        key_padding_mask = torch.ones(
            (batch_size, seqlen), dtype=torch.bool, device=key.device
        )
