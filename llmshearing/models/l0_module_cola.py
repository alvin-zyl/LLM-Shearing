...
class L0Module(nn.Module):
    def __init__(self, cfg, device):
        ...
        self.lagrangian_warmup_steps = Time.from_timestring(
            l0_module_cfg.lagrangian_warmup_steps
        ).value
        self.device = device
        ...
        # l0 params
        self.lambdas = {}
        self.lambdas["lambda_1"] = torch.nn.Parameter(
            torch.tensor([0.0], device=device)
        )
        self.lambdas["lambda_2"] = torch.nn.Parameter(
            torch.tensor([0.0], device=device)
        )
        self.masks = {}
    ...
    def initialize_one_module(self, module_name: str):
        ...
        try:
            ...
            raise NotImplementedError(
                "Instance `{}` does not implement `{}`".format(self, func_name)
            )
        method()
    ...
    def initialize_intermediate(self):
        ...
        if self.target_model_info is not None:
            ...
            pd = {
                ...
                "lambda_2_intermediate": torch.nn.Parameter(
                    torch.tensor([0.0], device=self.device)
                ),
            }
            self.lambdas.update(pd)
    ...
    def initialize_mlp_hidden(self):
        ...
        if self.target_model_info is not None:
            ...
            pd = {
                ...
                "lambda_2_mlp_hidden": torch.nn.Parameter(
                    torch.tensor([0.0], device=self.device)
                ),
            }
            self.lambdas.update(pd)

        mlp_hidden_mask = Mask(
            name="mlp_hidden",
            mask_shape=mask_shape,
            num_params_per_mask=num_params_per_mask,
            mask_output_shape=mask_output_shape,
            target_sparsity=target_mlp_hidden_sparsity,
            target_mask_size=target_mask_size,
            device=self.device,
            eval_target_model=self.eval_target_model,
        )
        self.masks["mlp_hidden"] = mlp_hidden_mask
    ...
    def get_expected_num_params(
        self, expected_scores: dict
        ...
        num_parameters += (
            torch.sum(int_score) * self.masks.intermediate.num_params_per_mask
        )
        num_parameters += (
            torch.sum(mlp_hidden_score) * self.masks.mlp_hidden.num_params_per_mask
        )
    ...
    def get_target_sparsity(self, pruned_steps: int, full_sparsity: float = None):
        ...
        if getattr(self, "lagrangian_warmup_steps", 0) > 0:
            target_sparsity = (target_sparsity - self.start_sparsity) * min(
                1, pruned_steps / self.lagrangian_warmup_steps
            ) + self.start_sparsity
        return target_sparsity

    def lagrangian_regularization(self, pruned_steps: int):
        def _lag_loss(
            expected_sparsity: torch.tensor,
            target_sparsity: float,
        ...
        expected_size = self.get_expected_num_params(
            expected_scores
        )  #! calculate \bar s
        ...
        if self.target_model_info is None:
            lagrangian_loss = _lag_loss(
                expected_sparsity,
                target_sparsity,
                self.lambdas["lambda_1"],
                self.lambdas["lambda_2"],
            )
            return_v = {
            ...
            for key in expected_sparsitys:
                return_v[f"expected_{key}_sparsity"] = (
                    expected_sparsitys[key].mean().item()
                )
        else:
            lagrangian_loss = 0
            return_v = {}
            for pruning_module in self.pruning_modules:
                ts = self.get_target_sparsity(
                    pruned_steps, self.masks[pruning_module].target_sparsity
                )
                ...
                return_v.update(
                    {
                        "expected_{}_sparsity".format(pruning_module): expected_ts,
                        "target_{}_sparsity".format(pruning_module): ts,
                    }
                )
...
def test_l0_module():
    ...
    cfg = om.load(
        "/scratch/gpfs/mengzhou/space2/examples/examples/llm/yamls/llama/7b.yaml"
    )
    cfg = om.load(
        "/scratch/gpfs/mengzhou/space2/examples/examples/llm/yamls/pythia/410m.yaml"
    )
