...
@contextmanager
def fsdp_state_dict_type_context(
    module: torch.nn.Module, state_dict_type: str = "full"
):
    ...
    # are formatted to for a single rank's FSDP module.
        ...
        raise NotImplementedError(
            f"No valid FSDP state_dict_type for {state_dict_type}"
        )
...
def fsdp_get_optim_state_dict(
    model: torch.nn.Module, optim: torch.optim.Optimizer, state_dict_type: str = "full"
) -> Dict[str, Any]:
    ...
    if not using_torch_2():
        optim_state_dict = _legacy_fsdp_get_optim_state_dict(
            model, optim, state_dict_type
        )
    else:
        with fsdp_state_dict_type_context(
            module=model, state_dict_type=state_dict_type
        ):
            optim_state_dict = FSDP.optim_state_dict(model, optim)  # type: ignore
    return optim_state_dict


def _legacy_fsdp_get_optim_state_dict(
    model: torch.nn.Module, optim: torch.optim.Optimizer, state_dict_type: str = "full"
) -> Dict[str, Any]:
    if version.parse(torch.__version__) < version.parse("1.13.0"):
        raise RuntimeError("To use FSDP with Composer, you must use torch>=1.13.0.")
...
def _legacy_optim_state_dict_to_load(
    ...
    if state_dict_type == "sharded":
        ...
        sharded_optim_state_dict = FSDP.scatter_full_optim_state_dict(
            full_optim_state_dict=optim_state_dict, model=model
        )
        return sharded_optim_state_dict


def get_fsdp_sharded_optim_state_dict(
    full_optim_state_dict: Dict[str, Any], model: torch.nn.Module
):
    if version.parse(torch.__version__) < version.parse("1.13.0"):
        raise RuntimeError("To use FSDP with Composer, you must use torch>=1.13.0.")
    from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
...
def get_fsdp_full_optim_state_dict(
    model: torch.nn.Module, optim: torch.optim.Optimizer, rank0_only: bool = True
):
    if version.parse(torch.__version__) < version.parse("1.13.0"):
        raise RuntimeError("To use FSDP with Composer, you must use torch>=1.13.0.")
    from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
...
class State(Serializable):
    ...
    def __init__(
        ...
        self.model = model
        self._original_model: ComposerModel = None
        ...
        if self.load_fsdp_monolith_rank0_only:
            ...
            if (
                ...
                and next(model.parameters()).device.type == "meta"
            ):
                rank0_on_meta = 1
            rank0_on_meta_tensor = self.device.tensor_to_device(
                torch.tensor([rank0_on_meta], dtype=torch.uint8)
            )
        ...
        self._ddp_sync_strategy: Union[str, DDPSyncStrategy] = (
            DDPSyncStrategy.SINGLE_AUTO_SYNC
        )
        self._train_data_spec: DataSpec = None
        self._backwards_create_graph: bool = False

    def _dataset_of(
        self, dataloader: Optional[Union[Evaluator, DataSpec, DataLoader, Iterable]]
    ) -> Optional[Dataset]:
    ...
    def max_duration(self, max_duration: Optional[Union[str, Time[int]]]):
        ...
        if max_duration.unit == TimeUnit.DURATION:
            raise ValueError(
                "TimeUnit.DURATION is not allowed as a unit for max_duration"
            )
    ...
    def schedulers(
        self,
        schedulers: Union[types.PyTorchScheduler, Sequence[types.PyTorchScheduler]],
    ):
    ...
    def fsdp_sharded_state_dict_enabled(self):
        return (
            ...
            and self.fsdp_state_dict_type in ["sharded", "local"]
        )
    ...
    def load_fsdp_monolith_rank0_only(self):
        return (
            self.fsdp_config is not None
            and self.fsdp_auto_wrap
            and self.fsdp_config["state_dict_type"] == "full"
            and self.fsdp_config["load_monolith_rank0_only"] == True
        )
    ...
    def get_model_state_dict(self) -> Dict[str, Any]:
        ...
        if self.fsdp_enabled and self.fsdp_state_dict_type is not None:
            with fsdp_state_dict_type_context(
                self.model, state_dict_type=self.fsdp_state_dict_type
            ):
        ...
        for attribute_name in self.serialized_attributes:
            ...
            if attribute_name == "dataset_state":
                ...
                if self.fsdp_enabled and self.fsdp_state_dict_type is not None:
                    ...
                    optim_state_dict = {
                        type(optimizer).__qualname__: optimizer.state_dict()
                    }
                ...
                serialized_value = [
                    (type(obj).__qualname__, obj.state_dict())
    ...
    def _apply_required_algorithms(
        ...
        for algo_name, serialized_value in state_dict["algorithms"]:
            ...
            if (
                hasattr(algorithms, algo_name)
                and getattr(algorithms, algo_name).required_on_load()
            ):
                ...
                if exclude_algorithms is None or algo_name not in exclude_algorithms:
                    ...
                    if (
                        type(algo) in current_algos
                        and not serialized_value["repr"] in current_algos[type(algo)]
                    ):
                        warnings.warn(
                            textwrap.dedent(
                                f"required_on_load algorithm {serialized_value['repr']} was enabled when training the "
    ...
    def load_model_state(
        ...
        if "algorithms" in state_dict:
            self._apply_required_algorithms(
        ...
        try:
            ...
            if model_on_rank:
                if (
                    self.fsdp_enabled
                    and self.fsdp_state_dict_type is not None
                    and not self.load_fsdp_monolith_rank0_only
                ):
                    log.debug(
        ...
        if model_on_rank and len(unexpected_keys) > 0:
            if (
                self.fsdp_config is not None
                and self.fsdp_config["use_orig_params"]
                and self.fsdp_state_dict_type == "local"
            ):
                log.warning(
    ...
    def load_optim_state(self, state_dict: Dict[str, Any]):
        ...
        for optimizer in ensure_tuple(self.optimizers):
            ...
            skip_optimizer_load = (
                1
                if serialized_value is not None
                and type(optimizer).__qualname__ not in serialized_value
                else 0
            )
            ...
            optim_state_dict = (
                serialized_value[type(optimizer).__qualname__]
                if serialized_value is not None
                else None
            )
            if self.fsdp_enabled:
                ...
                log.debug(
                    f"Loading FSDP optimizer with fsdp_state_dict_type={self.fsdp_state_dict_type}"
                )
                ...
                if not using_torch_2() or self.load_fsdp_monolith_rank0_only:
                    ...
                    assert optim_state_dict is not None
                    with fsdp_state_dict_type_context(
                        module=self.model, state_dict_type=self.fsdp_state_dict_type
                    ):
                        optim_state_dict = (
                            FSDP.optim_state_dict_to_load(  #  type: ignore
                                optim_state_dict=optim_state_dict,
                                model=self.model,
                                optim=optimizer,
                            )
            ...
            if attribute_name == "dataset_state":
                ...
                for metric_name in state_field_value.keys():
                    ...
                    if isinstance(serialized_value[metric_name], Metric):
                        ...
                        serialized_value[metric_name]._state_dict_pre_hooks = (
                            OrderedDict()
                        )
                        ...
                        metric_state_dict = serialized_value[metric_name]["state_dict"]
                        metric_computed_field = serialized_value[metric_name][
                            "_computed"
                        ]
                        ...
                        if isinstance(metric_computed_field, np.ndarray):
                            metric_computed_field = torch.from_numpy(
                                metric_computed_field
                            )
                            metric_computed_device = serialized_value[metric_name].get(
                                "_computed_device", None
                            )
                            if metric_computed_device is not None:
                                metric_computed_field = metric_computed_field.to(
                                    metric_computed_device
                                )
                    else:
                        raise ValueError(
                    ...
                    missing_keys, unexpected_keys = state_field_value[
                        metric_name
                    ].load_state_dict(metric_state_dict, strict=False)
                ...
                for eval_key in state_field_value.keys():
                    ...
                    for metric_name in state_field_value[eval_key].keys():
                        ...
                        if isinstance(serialized_value[eval_key][metric_name], Metric):
                            # For checkpoints saved using Composer <= 0.13.5
                            serialized_value[eval_key][metric_name].persistent(
                                mode=True
                            )
                            # Add new attr in torch2
                            serialized_value[eval_key][
                            ...
                            eval_metric_state_dict = serialized_value[eval_key][
                                metric_name
                            ].state_dict()
                            eval_metric_computed_field = serialized_value[eval_key][
                                metric_name
                            ]._computed
                            ...
                            if isinstance(eval_metric_computed_field, np.ndarray):
                                eval_metric_computed_field = torch.from_numpy(
                                    eval_metric_computed_field
                                )
                                eval_metric_computed_device = serialized_value[
                                    eval_key
                                ][metric_name].get("_computed_device", None)
                                if eval_metric_computed_device is not None:
                                    eval_metric_computed_field = (
                                        eval_metric_computed_field.to(
                        ...
                        missing_keys, unexpected_keys = state_field_value[eval_key][
                            metric_name
                        ].load_state_dict(eval_metric_state_dict, strict=False)
                        state_field_value[eval_key][
                            metric_name
                        ]._computed = eval_metric_computed_field
                        state_field_value[eval_key][metric_name].persistent(mode=True)
                        self.device.module_to_device(
                            state_field_value[eval_key][metric_name]
                        )
                        if len(missing_keys) > 0:
                            warnings.warn(
    ...
    def set_dataloader(
        ...
        if dataloader is None:
            ...
            if dataloader_label is None:
                raise ValueError(
                    "If the `dataloader` is specified, then `dataloader_label` must not be None."
                )
    ...
    def dataloader_len(self, num_batches: Union[int, Time[int]]):
        ...
        if self._dataloader is None:
            raise RuntimeError(
                "`State.dataloader_len` cannot be set if the dataloader is not defined."
            )
        try:
            ...
            dataloader_len = None
        if (
            dataloader_len is not None
            and num_batches >= 0
            and int(num_batches) > dataloader_len
        ):
            warnings.warn(
                (
    ...
    def find_unused_parameters(self, find_unused_parameters: bool):
        self._find_unused_parameters = find_unused_parameters

    @property
    def ddp_sync_strategy(self):
        return self._ddp_sync_strategy

    @ddp_sync_strategy.setter
    def ddp_sync_strategy(
        self,
        ddp_sync_strategy: Union[str, DDPSyncStrategy],
    ):
        self._ddp_sync_strategy = ddp_sync_strategy

    @property
    def train_data_spec(self):
        return self._train_data_spec

    @train_data_spec.setter
    def train_data_spec(self, train_data_spec: DataSpec):
        self._train_data_spec = train_data_spec

    @property
    def backwards_create_graph(self):
        return self._backwards_create_graph

    @backwards_create_graph.setter
    def backwards_create_graph(self, backwards_create_graph: bool):
        self._backwards_create_graph = backwards_create_graph
